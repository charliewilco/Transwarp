...
Transwarp is a standalone Mac app that lets CI dispatch a build to your own desktop through Cloudflare Tunnel. The app owns local lifecycle and visibility; the bundled Go runner owns the loopback HTTP API, Cloudflare connector process, registration hooks, and build execution.

## Shape

- Native SwiftUI macOS app, package-first and no XcodeGen.
- Go runner for the CLI/server process.
- Packaged for modern Apple Silicon Macs; the local runner refuses unsupported host OS, architecture, or macOS versions at startup.
- Registration advertises the runner's macOS, Apple Silicon architecture, CPU/memory basics, Xcode version, Developer directory, active build count, and queued build count so CI can choose real hardware instead of an opaque URL.
- `cloudflared` runs as an outbound connector to Cloudflare Tunnel and can be bundled into the app resources.
- Runner binds to loopback only.
- CI calls allowlisted job IDs, not arbitrary shell commands. Job IDs may contain only letters, numbers, dots, underscores, or hyphens, with a 128-byte limit.
- Jobs can clone an allowlisted repository plus explicit ref or commit into a per-build local workspace.
- Build output streams back as newline-delimited JSON.
